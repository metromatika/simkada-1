...
@foreach ($actions as $action)
  @if ($action->renderIf($model, $this))
    ...
    data-bs-original-title="Delete TPS"
      ...
      <i data-bs-toggle="tooltip" data-bs-original-title="{{ __('Delete') }}" class="{{ $action->icon }}mr-3 h-4 w-4 text-gray-600 group-hover:text-gray-700"></i>
      {{ $action->title }}
    </button>
  @endif
@endforeach
